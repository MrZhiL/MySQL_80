...
/*
	...
	3. 向books表中插入记录
	 ...
	 16. 查询书名和类型，其中note值为novel显示小说，law显示法律，medicine显示医药，cartoon显示卡通，	oke显示笑话
...
# 8. 统计书名中包含a字母的书的数量和库存总量
SELECT name, num FROM books WHERE name LIKE '%a%';
...
# 18. 统计每一种note的库存量，并合计总量
SELECT IFNULL(note, '合计总库存量') note, SUM(num)
FROM books
GROUP BY note
WITH ROLLUP;

# 19. 统计每一种note的数量，并合计总量
SELECT IFNULL(note, '合计总类型') note, COUNT(note)
FROM books
GROUP BY note
WITH ROLLUP;
